...
public class GravityBody : MonoBehaviour
{
    ...
    // ... tes méthodes AddGravityArea et RemoveGravityArea ...

    // ---------------------------------------------------------
    // --- OUTILS POUR L'ÉDITEUR (LEVEL DESIGN) ---
    // ---------------------------------------------------------
#if UNITY_EDITOR
    [ContextMenu("Align to Nearest Gravity")]
    private void AlignInEditor()
    {
        // 1. On cherche toutes les zones de gravité de la scène
        GravityArea[] allAreas = FindObjectsOfType<GravityArea>();

        if (allAreas.Length == 0)
        {
            Debug.LogWarning("Aucune GravityArea trouvée dans la scène !");
            return;
        }

        GravityArea closestArea = null;
        float minDistance = float.MaxValue;

        // 2. On trouve la zone de gravité la plus proche de cet objet
        foreach (GravityArea area in allAreas)
        {
        ...
        if (closestArea != null)
        {
            // Cette ligne est cruciale : elle permet d'utiliser Ctrl+Z si l'alignement ne te plaît pas !
            UnityEditor.Undo.RecordObject(transform, "Align Object to Gravity");

            Vector3 gravityDir = closestArea.GetGravityDirection(this).normalized;
            Vector3 targetUp = -gravityDir;

            // On oriente l'objet
            transform.rotation = Quaternion.FromToRotation(transform.up, targetUp) * transform.rotation;

            Debug.Log($"<b>{gameObject.name}</b> a été aligné avec succès sur la gravité de <b>{closestArea.gameObject.name}</b>.", this);
        }
    }
#endif
}